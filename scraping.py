...
def get_schedule(season: str, team_name: str) -> Schedule:
    global season_map, season_id, base_url
    # if filename := check_season_and_team(season_id, season):
    #     try:
    #         with open(filename, 'r') as jsonfile:
    #             data = json.load(jsonfile)
    #             print(data)
    #     except FileNotFoundError:
    #         create_master_json(season_id, season)
    # else:
    #     create_master_json(season_id, season)

    driver = webdriver.Chrome(options=opts, executable_path=chrome_driver)
    try:
        url = base_url.format(season_map[season.upper()], season_id)
        driver.get(url)
...
def get_schedules(seasons: list[str], teams: list[str]) -> list[Schedule]:
    schedules = []
    for i, (season, team) in enumerate(zip(seasons, teams)):
...
def get_master_json(ses_id: int, season_name: str) -> str | None:
    global folder_path

    if not os.path.exists(f"{folder_path.format(ses_id)}/{season_name}/master.json"):
        os.makedirs(f"{folder_path.format(ses_id)}/{season_name}", exist_ok=True)
        create_master_json(ses_id, season_name)

    return f"{folder_path.format(ses_id)}/{season_name}/master.json"


def create_master_json(ses_id: int, season_name: str) -> None:
    global season_map, season_id, base_url
    driver = webdriver.Chrome(options=opts, executable_path=chrome_driver)
    try:
        url = base_url.format(season_map[season_name.upper()], ses_id)
        driver.get(url)
        soup_file = driver.page_source
        soup = BeautifulSoup(soup_file, features="html.parser")
        driver.quit()
...
if __name__ == '__main__':
    ...
    create_master_json(season_id, s)
    # week1 = sch[0]
